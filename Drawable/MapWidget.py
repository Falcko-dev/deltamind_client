...
class MapWidget(QWidget):
	...
	def __init__(self, window_master, map_id, map_title, map_author, date, is_on_search_tab):
		...
		self.setMaximumSize(781*10, 120)
		self.setMinimumSize(int(781), int(120))
		if is_on_search_tab:
		...
		width_ = self.window_master.ui.searchMapsScroll.width() - 40
		self.resize(width_, self.height())
		self.ui.horizontalLayout.setGeometry(QRect(self.ui.horizontalLayout.geometry().x(),
												   self.ui.horizontalLayout.geometry().y(),
												   width_, self.height()))
		self.ui.horizontalLayoutWidget.resize(width_, self.height())
		self.show()